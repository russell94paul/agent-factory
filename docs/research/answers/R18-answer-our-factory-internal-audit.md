...
grain           agent-factory       {blueprints/windsorai_client_a.yaml,
...
| **F32** | "`windsorai_client_a.yaml`'s `primary_key` still doesn't match" — `lane/certify` | "A retry loop with no attempt cap" — `lane/control-plane` |
...
| 40 | Whether `QA_DG1_GEP_PREFECT_PR` is a share-consumed (imported) database | **NOT-DETERMINABLE** | `blueprints/windsorai_client_a.yaml:15` records the name and nothing else | `SHOW DATABASES LIKE 'QA_DG1_GEP_PREFECT_PR'` (an imported DB reports an `origin`), or a declared `share_consumed:` field. **Needs a credential — not requested** |
