...
|---|---|---|
| **Job / Outcome** | 🟡 EXISTS — **outside this repo** | ⚠ **Corrected 2026-08-30.** This row read `⛔ MISSING — nothing binds a client ticket to its lanes, its contract, its evidence and its approvals.` That is false about the estate. `clients/GEP/tickets/<ticket>/artifact.yaml` binds exactly that, in production, written by `~/.claude/commands/gep-feature.md`: `ticket`, `stage`, `stage_history[]`, `branch`, `pr_url`, per-layer `changes{eclipse,pbi_model,snowflake}`, `requirements`, `decisions`, `change_requests`, `deploy_history{env}`, `jira_sync`, `comments_posted`. Two live instances (`GP-197`, `GP-199`). **What is actually missing is smaller and more specific: it carries no evidence *class* and no *verdict*.** So it cannot say which of TARGET/CONSUMER/REGRESSION/ROLLBACK is absent, and `factory/` has never read it. The work is a reconciliation, not a design. `tasks.Task` is a title with an evidence list; `Lane` is a unit of *our own build work*; `Preset` maps a ticket **type** to a config and **is now consumed** — `factory/control.py` (RUN-03) reads it. The remaining gap is the join between `artifact.yaml` and the contract, not the absence of an object. |
