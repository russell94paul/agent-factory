...
## V2 `.agent-platform` delta

A later review found that the `.agent-platform/bootstrap/` pack had been read previously but many of its concrete mechanisms were only represented at bundle level in the canonical concept index. V2 preserves the deadline architecture and adds a targeted delta under `11_AGENT_PLATFORM_DELTA/`.

**Immediate change:** treat Execution Surface Routing as a near-term scheduler constraint so tasks can be routed to compatible local/cloud/worktree surfaces without creating a second orchestration system.

**Strategic additions preserved for later:** certified capability records, Synthesis Inbox/Promotion Board, mission assembly, compute/integration contracts, and the Venture -> Customer Learning -> Portfolio loop.

Read `11_AGENT_PLATFORM_DELTA/CRUCIAL_FEATURES_DELTA.md` before the Claude Plan Mode prompt.
