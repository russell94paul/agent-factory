...
<!-- MERGE NOTE 2026-08-30 — union, not a choice between the two.
     Both blocks below were appended independently: F50-F53 by the artifact lane (renumbered from
     its own F11-F13 in 96cff57, per the block convention) and F11-F34 by the control-plane lane.
     Neither is a subset of the other, so both are kept.

     ⚠ THE IDS COLLIDE ACROSS TWO FILES, and this merge does NOT resolve that.
     `docs/findings.d/F20-*` and `F21-*` are "gate finishes can never pass" and "gate succeeds is
     an all-time ratio". The F20 and F21 below are different findings entirely — "an instrument
     that counts its own writes" and "pipelines.py has no in-process lock". Same ids, different
     content, different files. Renumbering 24 entries inside a merge commit would bury a semantic
     change in a mechanical one, so the collision is marked here rather than silently fixed. It is
     the reason docs/findings.d/ exists (one file per finding, so separate paths cannot conflict);
     findings.md is the legacy ledger and `load()` reads both. -->
